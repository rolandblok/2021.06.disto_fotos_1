...
try:
    for laser_foto in laser_photos:
        ...
        new_row = {'file':laser_foto.filename, 'a':disto_transform_model.a, 'b':disto_transform_model.b, 'c':disto_transform_model.c,
                    ...
                    "max_residual[px]":max_distances}
        result = pandas.concat([result, pandas.DataFrame(new_row, index=[0])], ignore_index=True)

        # result = result.concat(, ignore_index=True)
except Exception as e:
    track = format_exc()
    print(track)
finally:
    result.to_csv("disto_results_2023nov.csv", index=False)
